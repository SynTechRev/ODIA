"""JAMS quarterly consumer arbitration data retrieval client.

CCP § 1281.96 requires JAMS to publish quarterly consumer arbitration
statistics.  JAMS publishes this data at jamsadr.com, historically as Excel
or CSV files linked from a statistics landing page.

Design mirrors retrieval_aaa.py: parse_html_table() and parse_csv_bytes()
accept pre-fetched content so that all unit tests run against a captured
fixture and never hit the live site.
"""
...
_PROVIDER = "JAMS"
_BASE_URL = "https://www.jamsadr.com"
_STATS_PATH = "/consumer-arbitration-statistics"
_USER_AGENT = (
...
# JAMS column header map -- JAMS uses slightly different labels than AAA
_COL_MAP: dict[str, str] = {
    ...
    "date of closing": "disposition_date",
    "name of business": "non_consumer_party_name",
    "business": "non_consumer_party_name",
    "respondent name": "non_consumer_party_name",
...
class JAMSRetriever:
    """Retrieve and parse JAMS quarterly consumer arbitration data."""
    ...
    def parse_html_table(
        ...
        """Parse an HTML page containing a JAMS case-data table.

        Pass fixture HTML in tests -- no network access occurs.
        """
    ...
    def parse_csv_bytes(
        ...
        """Parse JAMS CSV bytes into NormalizedCase records."""
    ...
    def fetch_quarterly_index(
        ...
        if html_source is None:
            ...
            self._save_snapshot(html_source, "jams_index.html")
    ...
    def download_and_parse(
        ...
        if release.fmt == "html_table":
            ...
            self._save_snapshot(
                resp.text, f"jams_{release.year}_q{release.quarter}.html"
            )
        ...
        self._save_snapshot(
            f"URL: {release.url}\n", f"jams_{release.year}_q{release.quarter}_meta.txt"
        )
        if release.fmt == "excel":
            return self._parse_excel_bytes(
                content, release.year, release.quarter, release.url, entity_registry
            )
    ...
    def _parse_index(self, html: str) -> list[JAMSRelease]:
        soup = BeautifulSoup(html, "html.parser")
        releases: list[JAMSRelease] = []
